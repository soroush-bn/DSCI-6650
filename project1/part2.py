...
def main():
    ...
    parser.add_argument('--non_stationary', type=str, default="gradual" ,help='activate non-stationary bandit, value can be gradual or abrupt' )
    parser.add_argument('--gradual_type', type=str, default="drift" ,help='type of gradual change(if any), value can be drift or revert' )
    ...
    for i in range(num_problems):
        seed+=1
        np.random.seed(seed)
        optimistic_greedy_rewards , _ = bandit.run(steps, "optimistic","greedy",non_stationary=non_stationary,gradual_type=gradual_type)
        optimistic_greedy_average_rewards[i] = np.mean(optimistic_greedy_rewards)
        e_greedy_fixed_rewards , _ = bandit.run(steps, "zero","epsilon_greedy",non_stationary=non_stationary,gradual_type=gradual_type)
        e_greedy_fixed_average_rewards[i] = np.mean(e_greedy_fixed_rewards)
        e_greedy_decreasing_rewards , _ = bandit.run(steps, "zero","epsilon_greedy",decreasing=True,non_stationary=non_stationary,gradual_type=gradual_type)
